...
struct Widget
{
	...
	bool draw_icon;
	BBox icon_atlas_box;
	ivec2 icon_offset; // Icon offset from bottom-left of box (After offset_line_bot/top was applied)
	int offset_line_bot;
	int offset_line_top;
...
void ui_system_initialize(Glyph_Atlas_* glyph_atlas, Bitmap_Atlas_Writer* atlas_writer)
{
	...
	ui_system.max_descender_height = glyph_atlas->max_descender_height;
	...
	auto sdf_down_triangle = [](vec2 pos, int pixel_size) -> float {
		float pixel_width = 1.0f / pixel_size;
		float r = pixel_width * 2; // Radius doesn't really mean much for triangle sdf
		float max = 1.0f - (r + pixel_width); // Keep outline 1 pixel width away from border (For anti-aliasing)
		vec2 a = vec2(-max, max);
		vec2 b = vec2(0.0f, -max);
		vec2 c = vec2(max, max);
	...
	ui_system.atlas_box_down_triangle = bitmap_atlas_writer_add_sdf_symbol(atlas_writer, triangle_size, sdf_down_triangle);
...
Widget_Handle ui_system_add_widget(bool is_hidden = false)
{
...
bool ui_system_push_button(const char* label_text)
{
	Widget_Handle handle = ui_system_add_widget();
	...
	// Set widget 'Style'
	{
		...
		widget.max_width = BUTTON_WANTED_CHAR_COUNT * ui_system.char_size.x + TEXT_BORDER_SPACE;
		widget.height = ui_system.line_item_height;
		widget.can_grow_beyond_max_width = true;
...
void ui_system_push_label(const char* text, bool restrain_label_size)
{
	Widget_Handle handle = ui_system_add_widget();
	Widget& widget = ui_system.widgets[handle.widget_index];
	...
	// Set widget 'Style'
	{
		...
		// Size options
		widget.height = ui_system.line_item_height;
		widget.can_grow_beyond_max_width = false;
		if (restrain_label_size) {
			widget.min_width = LABEL_CHAR_COUNT_SIZE * ui_system.char_size.x;
			widget.max_width = LABEL_CHAR_COUNT_SIZE * ui_system.char_size.x;
			widget.can_combine_in_lines = true;
		}
		else {
			widget.min_width = widget.text_display.length * ui_system.char_size.x;
			widget.max_width = widget.min_width;
			widget.can_combine_in_lines = false;
		}

		// Input options
		widget.is_clickable = false;
		widget.can_obtain_text_input = false;
		widget.text_input_type = Text_Input_Type::TEXT;
	}
}
...
Text_Input_State ui_system_push_text_input(String text)
{
	...
	// Set widget 'Style'
	{
		...
		widget.focused_color = COLOR_FOCUSED_BG;
		widget.text_alignment = Text_Alignment::LEFT;
		...
		widget.max_width = TEXT_INPUT_MAX_CHAR_COUNT * ui_system.char_size.x + TEXT_BORDER_SPACE;
		widget.height = ui_system.line_item_height;
		widget.can_grow_beyond_max_width = true;
		widget.can_combine_in_lines = true;
...
int ui_system_push_int_input(int value)
{
	...
	widget.background_color = COLOR_INPUT_BG_NUMBER;
...
bool ui_system_push_checkbox(bool enabled)
{
	...
	widget.icon_offset = ivec2(BORDER_SPACE + CHECKBOX_PADDING);
...
UI_Subsection_Info ui_system_push_subsection(bool enabled, const char* section_name, bool own_scrollbar)
{
	...
	// Set widget 'Style'
	{
		...
		widget.border_color = COLOR_BUTTON_BORDER;
		widget.text_alignment = Text_Alignment::LEFT;
		widget.text_display = ui_system_add_string(string_create_static(section_name));
		widget.draw_icon = true;
		widget.icon_atlas_box = enabled ? ui_system.atlas_box_down_triangle : ui_system.atlas_box_left_triangle;
		...
		widget.icon_offset = ivec2(BORDER_SPACE + PAD_LEFT_RIGHT, free_space / 2);
		widget.offset_line_bot = 0;
		widget.offset_line_top = 0;

		// Size options
		widget.min_width = 0;
		widget.max_width = 0; // Doesn't matter, as widget cannot be combined in lines
		widget.height = ui_system.line_item_height;
		widget.can_grow_beyond_max_width = true;
		widget.can_combine_in_lines = false;
...
void container_element_gather_width_information_recursive(Container_Element* element, bool collapse_allowed)
{
	...
	if (element->is_widget)
	{
		...
		element->min_width_for_line_merge = widget.max_width;
		element->width_can_grow_beyond_max = widget.can_grow_beyond_max_width;
		element->height_can_grow = false;
		element->min_height = widget.height;
		element->wanted_height = widget.height;
...
)
{
	...
	if (element->is_widget)
	{
		...
		box.min.y += y_offset;
		box.max.y += y_offset;
		if (!widget.can_grow_beyond_max_width) {
			box.max.x = box.min.x + widget.max_width;
		}
		...
		if (widget.draw_background)
		{
			if (widget.can_obtain_text_input && element->element_index == ui_system.focused_widget_index) {
				bg_color = widget.focused_color;
			}
			else if (widget.is_clickable && element->element_index == ui_system.mouse_hover_widget_index) {
		...
		if (widget.draw_icon) {
			mesh_push_subimage_clipped(mesh, box.min + widget.icon_offset, widget.icon_atlas_box, glyph_atlas->bitmap_atlas_size, clipping_box);
			text_pos.x += widget.icon_atlas_box.max.x - widget.icon_atlas_box.min.x + PAD_LEFT_RIGHT + widget.icon_offset.x;
		}
...
void imgui_test_entry()
{
	...
	bool subsection_status = true;
	bool subsection_breakpoints = true;
	bool subsection_watch_values = true;
	Dynamic_Array<Watch_Value> watch_values = dynamic_array_create<Watch_Value>();
