...
Argument_Info argument_info_make(
    ...
    Optional<String*> argument_name = optional_make_failure<String*>(),
    Argument_State state = Argument_State::NOT_ANALYSED,
...
void expression_info_set_dot_call(Expression_Info* info, AST::Expression* first_argument, Operator_Overload* overload)
{
    info->result_type = Expression_Result_Type::DOT_CALL;
    info->options.dot_call.first_argument = first_argument;
    assert(overload != 0, "");
    info->options.dot_call.is_polymorphic = overload->is_polymorphic;
    if (overload->is_polymorphic) {
        info->options.dot_call.options.polymorphic.base = overload->options.polymorphic;
        info->options.dot_call.options.polymorphic.instance = nullptr;
    }
    else {
        info->options.dot_call.options.function = overload->options.function;
    }
...
void analysis_workload_entry(void* userdata)
{
    ...
    case Analysis_Workload_Type::MODULE_ANALYSIS:
    {
        ...
        if (module_node->context_changes.size != 0) 
        {
            ...
            // Create new workload
            Workload_Operator_Context_Change* context_change = workload_executer_allocate_workload<Workload_Operator_Context_Change>(nullptr, workload->current_pass);
    ...
    case Analysis_Workload_Type::DEFINITION:
    {
        ...
        else // Comptime definition
        {
            ...
            case Expression_Result_Type::DOT_CALL: {
                symbol->type = Symbol_Type::ERROR_SYMBOL;
                log_semantic_error("Cannot use dot-call in definition", AST::upcast(definition->value_node));
                break;
            }
            case Expression_Result_Type::NOTHING: {
                symbol->type = Symbol_Type::ERROR_SYMBOL;
...
Cast_Mode operator_context_get_cast_mode_option(Operator_Context* context, Context_Option option) {
    assert((int)option >= CONTEXT_OPTION_CAST_MODE_START && (int)option <= CONTEXT_OPTION_CAST_MODE_END, "");
    return context->cast_mode_settings[(int)option - CONTEXT_OPTION_CAST_MODE_START];
}
...
Optional<Overload_Candidate> overload_candidate_try_create_from_expression_info(Expression_Info& info, Symbol* origin_symbol = 0)
{
    ...
    case Expression_Result_Type::DOT_CALL: {
        ...
        if (dot_call.is_polymorphic) {
            callable.type = Callable_Type::POLYMORPHIC;
            callable.options.polymorphic.is_function = true;
            callable.options.polymorphic.base = &dot_call.options.polymorphic.base->base_info;
            callable.options.polymorphic.options.polymorphic_function = dot_call.options.polymorphic.base;
            callable.parameter_count = dot_call.options.polymorphic.base->base_info.parameters.size;
            if (dot_call.options.polymorphic.base->base_info.return_type_node != 0) {
                callable.parameter_count -= 1; // Return type is included in polymorphic parameter array, but we don't want it for matching
            }
        }
        else {
            callable.type = Callable_Type::FUNCTION;
            callable.options.function = dot_call.options.function;
            candidate.function_signature = dot_call.options.function->signature;
        }
        break;
    }
    default: panic("");
    ...
    Parser::Section error_report_section = Parser::Section::WHOLE)
{
    ...
    if (argument_infos.size > callable.parameter_count) {
        if (log_errors_and_set_info) {
            log_semantic_error("Too many arguments were supplied", error_report_node, error_report_section);
            log_error_info_argument_count(argument_infos.size, callable.parameter_count);
        }
        return false;
    }
    ...
    if (log_errors_and_set_info) {
        for (int i = 0; i < unnamed_argument_count; i++) {
            auto info = argument_infos[i];
            info->parameter_index = i;
        }
    }
    ...
    for (int i = unnamed_argument_count; i < argument_infos.size; i++)
    {
        // Find parameter with same base_name
        auto argument_info = argument_infos[i];
        assert(argument_info->argument_name.available, "");
        auto id = argument_info->argument_name.value;
        ...
        for (int j = 0; j < callable.parameter_count; j++)
        {
            ...
            match_found = true;
            if (log_errors_and_set_info) {
                argument_info->parameter_index = j;
            }
            ...
            if (j < unnamed_argument_count) {
                error_occured = true;
                if (log_errors_and_set_info) {
                    log_semantic_error("Named argument overlaps with previously given unnamed argument", upcast(argument_info->expression));
                }
            }
            break;
        }
        ...
        if (!match_found) {
            error_occured = true;
            if (log_errors_and_set_info) {
                log_semantic_error("No parameter matches this argument's name", upcast(argument_info->expression), Parser::Section::IDENTIFIER);
            }
        }
    }
...
bool argument_nodes_match_to_parameters(
    ...
    Parser::Section error_report_section = Parser::Section::WHOLE)
{
    Array<Argument_Info*> infos;
    if (arguments.size == 0) {
        infos.size = 0;
        infos.data = nullptr;
    }
    else {
        infos = array_create_empty<Argument_Info*>(arguments.size);
    }
    SCOPE_EXIT(if (infos.data != nullptr) { array_destroy(&infos); });
    for (int i = 0; i < arguments.size; i++) {
        auto& arg = arguments[i];
        Argument_Info* info = pass_get_node_info(semantic_analyser.current_workload->current_pass, arg, Info_Query::CREATE_IF_NULL);
        *info = argument_info_make(arg->value, arg->name);
        infos[i] = info;
    }

    return arguments_match_to_parameters(infos, callable, unnamed_argument_count, log_errors_and_set_info, error_report_node, error_report_section);
}


void arguments_analyse_in_unknown_context(Array<Argument_Info*> infos) {
    for (int i = 0; i < infos.size; i++) {
        auto arg_info = infos[i];
        if (arg_info->state != Argument_State::NOT_ANALYSED) {
            // Arg info expressions may be nullpointer since #instanciate exists
            if (arg_info->expression != nullptr) {
                auto value_opt = pass_get_node_info(semantic_analyser.current_workload->current_pass, arg_info->expression, Info_Query::TRY_READ);
                assert(value_opt != 0, "Already analysed must mean that expression info for value exists!");
            }
            continue;
        }
        semantic_analyser_analyse_expression_value(arg_info->expression, expression_context_make_unknown());
    }
}

void arguments_analyse_in_unknown_context(Dynamic_Array<AST::Argument*>& arguments, Array<Datatype*> known_parameter_types = array_create_empty<Datatype*>(0)) 
{
    bool encountered_named_argument = false;
    for (int i = 0; i < arguments.size; i++) {
        auto arg = arguments[i];
        auto arg_info = pass_get_node_info(semantic_analyser.current_workload->current_pass, arg, Info_Query::TRY_READ);
        ...
        if (known_parameter_types.data != 0 && i < known_parameter_types.size && !encountered_named_argument) {
            context = expression_context_make_specific_type(known_parameter_types[i]);
        }
    ...
    Dynamic_Array<Matching_Constraint>& constraints)
{
    ...
    else if (polymorphic_type->type == Datatype_Type::STRUCT_INSTANCE_TEMPLATE)
    {
        ...
        for (int i = 0; i < struct_instance.parent->info.parameter_nodes.size; i++) // Don't match implicit parameters, they should resolve by themselves
        {
            ...
            if (match_type->type == Datatype_Type::ERROR_TYPE) {
    ...
    int parameter_index) 
{
    ...
    if (state == Argument_State::ANALYSED && expression != nullptr) {
        result.argument_type = get_info(expression)->cast_info.type_afterwards;
    }
    else {
        result.argument_type = compiler.type_system.predefined_types.error_type;
    }
    return result;
}

Datatype* analyse_argument_if_not_already_done(Argument_Info* info, Expression_Context context)
{
    switch (info->state)
    {
    case Argument_State::ANALYSED: {
        break;
    }
    case Argument_State::CAST_MISSING: {
        info->state = Argument_State::ANALYSED;
        auto value_info = get_info(info->expression);
        value_info->cast_info = expression_context_apply(expression_info_get_type(value_info, true), context, true, info->expression);
        info->argument_type = expression_info_get_type(value_info, false);
        break;
    }
    case Argument_State::NOT_ANALYSED: {
        info->state = Argument_State::ANALYSED;
...
Instanciation_Result instanciate_polymorphic_callable(
    Callable_Polymorphic_Info poly_callable, Array<Argument_Info*> arguments, Expression_Context context, int unnamed_argument_count,
    AST::Node* instanciation_node, Parser::Section error_report_section = Parser::Section::WHOLE)
{
    auto& types = compiler.type_system.predefined_types;
    auto& poly_base = *poly_callable.base;
    ...
    if (!poly_callable.is_function)
    {
        ...
        if (workload->type == Analysis_Workload_Type::STRUCT_POLYMORPHIC || workload->type == Analysis_Workload_Type::FUNCTION_HEADER) {
            ...
            for (int i = 0; i < arguments.size; i++)
            {
                ...
                if (check_if_expression_contains_implicit_parameters(argument->expression)) {
        ...
        {
            for (int i = 0; i < instance_values.size; i++) {
                instance_values[i] = polymorphic_value_make_type(types.error_type);
            }
            ...
            for (int i = 0; i < arguments.size; i++)
            {
                ...
                else
                {
                    ...
                    auto constant_result = expression_calculate_comptime_value(argument->expression, "Struct arguments must be comptime");
                    if (constant_result.available)
                    {
                        ...
                        if (constant.type->type == Datatype_Type::TYPE_HANDLE)
                        {
                            ...
                            if (handle.index >= (u32) compiler.type_system.types.size) {
                                success = false;
                                log_semantic_error("Invalid constant type handle index", upcast(argument->expression));
                                continue;
                            }
    ...
    Datatype* final_return_type = types.error_type;
    if (return_type_index != -1)
    {
    ...
    {
        auto& argument = *arguments[i];
        if (argument.parameter_index < parameter_nodes.size) { // See how argument-to-parameter mapping is done to get this
            continue;
        }
        ...
        auto comptime_result = expression_calculate_comptime_value(
            argument.expression, "Parameter is polymorphic, but argument cannot be evaluated at comptime", &was_unavailable);
        if (comptime_result.available) 
        {
            ...
            if (implicit.defined_in_parameter_index == return_type_index) {
                ...
                arguments[implicit.defined_in_parameter_index]->reanalyse_param_type_flag = true;
            }
        }
        else {
            success = false;
    ...
    for (int i = 0; i < poly_base.parameter_analysis_order.size; i++)
    {
        ...
        Argument_Info* argument = 0;
        {
            if (parameter_index < unnamed_argument_count) {
                argument = arguments[parameter_index];
                assert(argument->parameter_index == parameter_index, "");
            }
            else
            {
                for (int j = unnamed_argument_count; j < arguments.size; j++) {
                    if (arguments[j]->parameter_index == parameter_index) {
                        argument = arguments[j];
                        break;
                    }
                }
                if (argument == 0) {
                    if (!base_parameter.is_comptime && base_parameter.infos.has_default_value) {
                        continue;
                    }
                }
                assert(argument != 0, "Should not happen after argument to parameter mapping");
            }
        }

        // Skip if we can just use base-analysis result
        if (!base_parameter.is_comptime && base_parameter.depends_on.size == 0 && !base_parameter.infos.type->contains_type_template) {
            auto type = analyse_argument_if_not_already_done(argument, expression_context_make_specific_type(base_parameter.infos.type));
            if (datatype_is_unknown(type) && argument->expression != nullptr) { // Here we have a special case for #instanciate to work (Non-comptime parameters are faked in argument-info)
                success = false;
            }
            continue;
        }
        ...
        argument->ignore_during_code_generation = parameter_node->is_comptime;
        ...
        if (base_parameter.depends_on.size > 0 || argument->reanalyse_param_type_flag)
        {
        ...
        // Note: I think I don't need the following anymore
        ...
        if (poly_callable.is_function)
        {
            ...
            Datatype_Function* instance_function_type = 0;
            {
                ...
                for (int i = 0; i < poly_base.parameters.size; i++)
                {
                    ...
                    if (base_param.depends_on.size > 0 || base_param.infos.type->contains_type_template) 
                    {
                        // If parameter has dependencies we need to find the instanciated type
                        parameter.has_default_value = false;
                        Argument_Info* argument = 0;
                        {
                            if (i < unnamed_argument_count) {
                                argument = arguments[i];
                                assert(argument->parameter_index == i, "");
                            }
                            else
                            {
                                for (int j = unnamed_argument_count; j < arguments.size; j++) {
                                    if (arguments[j]->parameter_index == i) {
                                        argument = arguments[j];
                                        break;
                                    }
                                }
                        ...
                        parameter.type = arguments[i]->argument_type;
                    }
                    else {
                        parameter.has_default_value = base_param.infos.has_default_value;
                        parameter.default_value_opt = base_param.infos.default_value_opt;
    ...
    if (poly_callable.is_function)
    {
        ...
        for (int i = 0; i < arguments.size; i++) {
            ...
            if (info->ignore_during_code_generation) {
                continue;
            }
            info->parameter_index = poly_base.parameters[info->parameter_index].options.index_in_non_polymorphic_signature;
        }
...
bool try_updating_expression_pointer_level(AST::Expression* expr, Datatype* deref_type, int given_pointer_level, int expected_pointer_level)
{
    ...
    bool auto_address_of = operator_context_get_boolean_option(context, Context_Option::AUTO_ADDRESS_OF);
...
#define EXIT_FUNCTION(function) expression_info_set_function(info, function); return info;
    ...
    case AST::Expression_Type::FUNCTION_CALL:
    {
        ...
        auto argument_infos_buffer = dynamic_array_create_empty<Argument_Info*>(call.arguments.size + 1);
        SCOPE_EXIT(dynamic_array_destroy(&argument_infos_buffer));
        Array<Argument_Info*> argument_infos = array_create_static(argument_infos_buffer.data, call.arguments.size);
        Argument_Info dot_call_first_parameter; // Needs to be on the stack

        // Initialize all arguments
        {
            auto& argument_nodes = call.arguments;
            for (int i = 0; i < argument_nodes.size; i++) {
                auto arg_info = get_info(argument_nodes[i], true);
                *arg_info = argument_info_make(argument_nodes[i]->value, argument_nodes[i]->name);
                dynamic_array_push_back(&argument_infos_buffer, arg_info);
            }
        }
        ...
        if (arguments_check_for_naming_errors(call.arguments, &unnamed_argument_count)) {
            ...
            arguments_analyse_in_unknown_context(argument_infos);
            EXIT_ERROR(types.error_type);
        }

        // Determine callable (Includes overload resolution)
        Callable callable;
        {
            ...
            else
            {
                ...
                // If dot call add additional argument info to array
                if (info->result_type == Expression_Result_Type::DOT_CALL) {
                    auto& dot_call = info->options.dot_call;
                    unnamed_argument_count += 1;
                    dot_call_first_parameter = argument_info_make(dot_call.first_argument, optional_make_failure<String*>(), Argument_State::ANALYSED, 0);
                    dynamic_array_insert_ordered(&argument_infos_buffer, &dot_call_first_parameter, 0);
                    argument_infos.data = argument_infos_buffer.data;
                    argument_infos.size += 1;
                }
            ...
            if (candidates.size != 1)
            {
                ...
                if (candidates.size > 1)
                {
                    const int named_argument_count = argument_infos.size - unnamed_argument_count;
                    for (int i = 0; i < candidates.size; i++)
                    {
                        auto& candidate = candidates[i];
                        if (!arguments_match_to_parameters(argument_infos, candidate.callable, unnamed_argument_count, false)) {
                ...
                if (candidates.size > 1)
                {
                    auto remove_candidates_based_on_better_type_match = [&](Dynamic_Array<Overload_Candidate>& candidates, Datatype* expected_type)
                    {
                        ...
                        bool matching_base_type_candidate_exists = false; // If a dereference/auto-address of would suffice
                        for (int j = 0; j < candidates.size; j++)
                        {
                            ...
                            if (types_are_equal(candidate.overloading_param_type, expected_type)) {
                                ...
                                int unused;
                                Datatype* expected_type_base = datatype_get_pointed_to_type(expected_type, &unused);
                                Datatype* given_type_base = datatype_get_pointed_to_type(candidate.overloading_param_type, &unused);
                                if (types_are_equal(expected_type_base, given_type_base)) {
                                    candidate.overloading_arg_can_be_cast = true;
                                    matching_base_type_candidate_exists = true;
                                }
                            }
                        }
                        ...
                        for (int j = 0; j < candidates.size; j++)
                        {
                            ...
                            if (candidate.overloading_arg_matches_type) {
                                ...
                                if (matching_candidate_exists || matching_base_type_candidate_exists) {
                                    remove = true;
                                }
                            }
                    ...
                    for (int i = 0; i < argument_infos.size && candidates.size > 1; i++)
                    {
                        auto arg_info = argument_infos[i];
                        ...
                        auto argument_type = semantic_analyser_analyse_expression_value(arg_info->expression, expression_context_make_unknown());
                        casts_are_missing = true;
                        arg_info->state = Argument_State::CAST_MISSING;
                ...
                else if (casts_are_missing)
                {
                    // Apply casts where necessary
                    for (int i = 0; i < argument_infos.size; i++)
                    {
                        auto arg_info = argument_infos[i];
                        if (arg_info->state == Argument_State::CAST_MISSING) {
                            auto expr_info = get_info(arg_info->expression);
                            assert(expr_info->cast_info.cast_type == Cast_Type::NO_CAST, "");
                            expr_info->cast_info = expression_context_apply(
                                expression_info_get_type(expr_info, true),
                                expression_context_make_specific_type(
                                    callable_get_parameter_info(candidates[0].callable, arg_info->parameter_index).type),
                                true, arg_info->expression
                            );
                            arg_info->state = Argument_State::ANALYSED;
            ...
            bool success = arguments_match_to_parameters(argument_infos, candidate.callable, unnamed_argument_count, true, upcast(expr), Parser::Section::ENCLOSURE);
            if (!success) {
                arguments_analyse_in_unknown_context(argument_infos);
                if (candidate.return_value_type.available) {
        ...
        // Callable-Type dependend processing (polymorphic instanciation or hardcoded function handling)
        switch (callable.type)
        {
        case Callable_Type::FUNCTION: {
        ...
        case Callable_Type::FUNCTION_POINTER: {
            info->specifics.function_call_signature = callable.options.function_pointer_type;
            break;
        }
        case Callable_Type::HARDCODED:
        {
            if (callable.options.hardcoded.type == Hardcoded_Type::TYPE_OF)
            {
            ...
            info->specifics.function_call_signature = callable.options.hardcoded.signature;
            break;
        }
        case Callable_Type::POLYMORPHIC:
        {
            ...
            Instanciation_Result instance_result = instanciate_polymorphic_callable(
                callable.options.polymorphic, argument_infos, context, unnamed_argument_count, upcast(expr), Parser::Section::ENCLOSURE
            );
        ...
        {
            auto arg_info = argument_infos[i];
            if (arg_info->ignore_during_code_generation) {
                continue;
            }

            // Check if already handled
            analyse_argument_if_not_already_done(arg_info, expression_context_make_specific_type(params[arg_info->parameter_index].type));
        }
    ...
    case AST::Expression_Type::INSTANCIATE:
    {
        auto& argument_nodes = expr->options.instanciate.arguments;
        if (argument_nodes.size == 0) {
            ...
            arguments_analyse_in_unknown_context(argument_nodes);
            EXIT_ERROR(types.error_type);
        }
        ...
        // Set argument info for first argument for analyse_in_unknown_context
        {
            auto info = get_info(argument_nodes[0], true);
            *info = argument_info_make(nullptr, optional_make_failure<String*>(), Argument_State::ANALYSED);
        }
        ...
        Polymorphic_Base_Info* poly_base = &expression_info->options.polymorphic_function.base->base_info;

        // Generate arguments for instanciation (This is a list so I can take pointers)
        List<Argument_Info> argument_list = list_create<Argument_Info>();
        SCOPE_EXIT(list_destroy(&argument_list));
        Dynamic_Array<Argument_Info*> argument_infos = dynamic_array_create_empty<Argument_Info*>(
            poly_base->parameters.size + poly_base->implicit_parameter_infos.size
        );
        SCOPE_EXIT(dynamic_array_destroy(&argument_infos));

        bool errors_occured = false;
        int unnamed_argument_count = 0;
        if (arguments_check_for_naming_errors(argument_nodes, &unnamed_argument_count)) {
            arguments_analyse_in_unknown_context(argument_nodes);
            EXIT_ERROR(types.error_type);
        }

        for (int i = 1; i < argument_nodes.size; i++) {
            auto argument_node = argument_nodes[i];
            if (!argument_node->name.available) {
                log_semantic_error("For #instanciate all arguments must have a name", upcast(argument_node), Parser::Section::FIRST_TOKEN);
                errors_occured = true;
            }
        }

        auto find_and_add_parameter = [&](String* id, int parameter_index) 
        {
            AST::Argument* argument_node = nullptr;
            // Note: We start at index 1, as the first argument is always the function to instanciate
            for (int j = 1; j < argument_nodes.size; j++) 
            {
                auto arg = argument_nodes[j];
                if (arg->name.available && id) {
            ...
            if (argument_node == 0) {
                log_semantic_error("Instanciate is missing an argument", expr, Parser::Section::ENCLOSURE);
                errors_occured = true;
            ...
            auto arg_info = get_info(argument_node, true);
            *arg_info = argument_info_make(argument_node->value, argument_node->name, Argument_State::NOT_ANALYSED, parameter_index);
            dynamic_array_push_back(&argument_infos, arg_info);
        };
        ...
        for (int i = 0; i < poly_base->parameter_nodes.size; i++)
        {
            ...
            if (!poly_parameter.is_comptime) 
            {
                ...
                Argument_Info arg_info = argument_info_make(nullptr, poly_parameter.infos.name, Argument_State::NOT_ANALYSED, i);
                arg_info.expression = nullptr; // It should be the case that setting the state to Analysed makes sure that expression is never used
                arg_info.state = Argument_State::ANALYSED;
                arg_info.argument_type = types.error_type; // Normally this value is never used
                auto list_node = list_add_at_end(&argument_list, arg_info);
                dynamic_array_push_back(&argument_infos, &list_node->value);
            }
            else
            {
                assert(poly_parameter.infos.name.available, "I think all poly parameters must have names!");
                find_and_add_parameter(poly_parameter.infos.name.value, i);
            }
        }
        ...
        for (int i = 0; i < poly_base->implicit_parameter_infos.size; i++) {
            ...
            find_and_add_parameter(implicit.id, poly_base->parameter_nodes.size + i); // Wacky index calculation, this is how extra "implicit" parameters can be specified
        }
        ...
        poly_info.base = poly_base;
        poly_info.is_function = true;
        poly_info.options.polymorphic_function = expression_info->options.polymorphic_function.base;
        Array<Argument_Info*> infos_array = array_create_static(argument_infos.data, argument_infos.size);
        auto result = instanciate_polymorphic_callable(poly_info, infos_array, expression_context_make_unknown(), 0, upcast(expr));
    ...
    case AST::Expression_Type::ARRAY_ACCESS:
    {
        ...
        auto array_type = semantic_analyser_analyse_expression_value(access_node.array_expr, expression_context_make_unknown());
        Datatype* initial_array_type = array_type;

        int array_pointer_level = 0;
        while (array_type->type == Datatype_Type::POINTER) {
            array_type = downcast<Datatype_Pointer>(array_type)->points_to_type;
            array_pointer_level += 1;
        }
        ...
        if (!type_is_valid)
        {
            ...
            if (overload != 0)
            {
                assert(!overload->is_polymorphic, "");
                semantic_analyser_analyse_expression_value(
                    access_node.index_expr, expression_context_make_specific_type(upcast(overload->options.function->signature->parameters[1].type))
                );
                type_is_valid = true;
                expected_pointer_level = overload->left_pointer_level;
                result_type = overload->options.function->signature->return_type.value;
                info->specifics.overload.function = overload->options.function;
            }
        }
        ...
        if (!type_is_valid && array_type->type == Datatype_Type::STRUCT)
        {
            ...
            if (struct_type->workload != 0 && struct_type->workload->polymorphic_type == Polymorphic_Analysis_Type::POLYMORPHIC_INSTANCE)
            {
                ...
                {
                    assert(overload->is_polymorphic, "Must be the case for base structure");

                    array_pointer_level = expected_pointer_level;
                    Argument_Info array_argument = argument_info_make(access_node.array_expr, optional_make_failure<String*>(), Argument_State::ANALYSED, 0);
                    Argument_Info index_argument = argument_info_make(access_node.index_expr, optional_make_failure<String*>(), Argument_State::NOT_ANALYSED, 1);
                    Argument_Info* arguments[2];
                    arguments[0] = &array_argument;
                    arguments[1] = &index_argument;
                    Array<Argument_Info*> argument_array;
                    argument_array.data = &arguments[0];
                    argument_array.size = 2;

                    Callable_Polymorphic_Info poly_info;
                    poly_info.base = &overload->options.polymorphic->base_info;
                    poly_info.is_function = true;
                    poly_info.options.polymorphic_function = overload->options.polymorphic;

                    Instanciation_Result result = instanciate_polymorphic_callable(poly_info, argument_array, context, 2, upcast(expr), Parser::Section::ENCLOSURE);
                    if (result.type == Instanciation_Result_Type::FUNCTION) {
                        type_is_valid = true;
        ...
        if (type_is_valid) {
            if (!try_updating_expression_pointer_level(access_node.array_expr, array_type, array_pointer_level, expected_pointer_level)) {
                type_is_valid = false;
            }
        }
        if (!type_is_valid) {
    ...
    case AST::Expression_Type::MEMBER_ACCESS:
    {
        ...
        case Expression_Result_Type::CONSTANT:
        {
            ...
            else if ((base_type->type == Datatype_Type::ARRAY || base_type->type == Datatype_Type::SLICE) && (member_node.name == ids.size || member_node.name == ids.data))
            {
            ...
            {
                int pointer_level = 0;
                auto type = datatype_get_pointed_to_type(base_type, &pointer_level);
                Overload_Key key = overload_key_make_call(type, member_node.name);
                auto& operator_context = semantic_analyser.current_workload->current_symbol_table->operator_context;
                Operator_Overload* overload = hashtable_find_element(&operator_context->operator_overloads, key);

                // Check for polymorphic overload if normal overload was not found
                if (overload == 0 && type->type == Datatype_Type::STRUCT)
                {
                    ...
                    if (structure->workload->polymorphic_type == Polymorphic_Analysis_Type::POLYMORPHIC_INSTANCE) {
                        ...
                        key.left_type = upcast(base_struct);
                ...
                if (overload != 0)
                {
                    ...
                    if (try_updating_expression_pointer_level(member_node.expr, type, pointer_level, overload->left_pointer_level)) {
                        if (overload->dot_call_as_member_access)
                        {
                            ModTree_Function* function = nullptr;
                            if (overload->is_polymorphic)
                            {
                                // Instanciate polymorphic function
                                Argument_Info argument_info = argument_info_make(member_node.expr, optional_make_failure<String*>(), Argument_State::ANALYSED, 0);
                                ...
                                poly_callable.base = &overload->options.polymorphic->base_info;
                                poly_callable.is_function = true;
                                poly_callable.options.polymorphic_function = overload->options.polymorphic;

                                Instanciation_Result instance_result = instanciate_polymorphic_callable(
                                    poly_callable, arguments, context, 1, upcast(expr), Parser::Section::WHOLE_NO_CHILDREN
                                );
                                ...
                                function = overload->options.function;
                            }
                            ...
                            expression_info_set_dot_call(info, member_node.expr, overload);
                            return info;
                        }
                    }
                }
    ...
    case AST::Expression_Type::UNARY_OPERATION:
    {
        ...
        case AST::Unop::NOT:
        {
            ...
            int pointer_level = 0;
            while (operand_type->type == Datatype_Type::POINTER) {
                pointer_level += 1;
                operand_type = downcast<Datatype_Pointer>(operand_type)->points_to_type;
            }

            // Check for primitive operand
            bool type_is_valid = false;
            int required_pointer_level = 0;
            Datatype* result_type = types.error_type;
            if (is_negate)
            {
            ...
            if (!type_is_valid)
            {
                ...
                if (overload != 0)
                {
                    ...
                    type_is_valid = true;
                    result_type = overload->options.function->signature->return_type.value;
                    required_pointer_level = overload->left_pointer_level;
                    info->specifics.overload.function = overload->options.function;
                    semantic_analyser_register_function_call(overload->options.function);
                }
            }
            ...
            if (type_is_valid)
            {
                if (!try_updating_expression_pointer_level(unary_node.expr, operand_type, pointer_level, required_pointer_level)) {
                    type_is_valid = false;
                }
            }
    ...
    case AST::Expression_Type::BINARY_OPERATION:
    {
        ...
        // Check if overload is a primitive operation (ints, floats, bools, pointers)
        if (types_are_equal(left_type, right_type))
        {
        ...
        if (!types_are_valid)
        {
            ...
            if (overload != nullptr)
            {
                ...
                info->specifics.overload.function = overload->options.function;
                info->specifics.overload.switch_left_and_right = overload->switch_left_and_right;
                semantic_analyser_register_function_call(overload->options.function);
        ...
        {
            bool auto_deref = operator_context_get_boolean_option(operator_context, Context_Option::AUTO_DEREFERENCE);
            bool auto_address_of = operator_context_get_boolean_option(operator_context, Context_Option::AUTO_ADDRESS_OF);

            if (!try_updating_expression_pointer_level(binop_node.left, left_type, left_type_pointer_level, expected_pointer_level_left)) {
                types_are_valid = false;
            }
            if (!try_updating_expression_pointer_level(binop_node.right, right_type, right_type_pointer_level, expected_pointer_level_right)) {
                types_are_valid = false;
            }
        }
    ...
    AST::Expression * expression, Datatype * source_type, Datatype * destination_type, Cast_Mode cast_mode)
{
    ...
    if (types_are_equal(destination_type, upcast(types.bool_type)))
    {
        ...
        if (source_type->type == Datatype_Type::POINTER) {
            allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::POINTER_TO_BOOL);
        }
        else if (source_type->type == Datatype_Type::VOID_POINTER) {
            allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::VOID_POINTER_TO_BOOL);
        }
        else if (source_type->type == Datatype_Type::FUNCTION) {
            allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::FUNCTION_POINTER_TO_BOOL);
        }
    ...
    // Check pointer operations + any cast
    {
        Cast_Mode allowed_mode = Cast_Mode::NONE;
        ...
        if (source_type->type == Datatype_Type::POINTER)
        {
            if (destination_type->type == Datatype_Type::VOID_POINTER) {
                ...
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::POINTER_TO_VOID_POINTER);
            }
            else if (destination_type->type == Datatype_Type::POINTER) {
                cast_type = Cast_Type::POINTERS;
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::POINTER_TO_POINTER);
            }
            else if (types_are_equal(destination_type, upcast(types.u64_type))) {
                cast_type = Cast_Type::POINTER_TO_U64;
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::POINTER_TO_U64);
            }
        ...
        else if (source_type->type == Datatype_Type::VOID_POINTER)
        {
            if (destination_type->type == Datatype_Type::POINTER) {
                cast_type = Cast_Type::POINTERS;
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::VOID_POINTER_TO_POINTER);
            }
            else if (destination_type->type == Datatype_Type::FUNCTION) {
                cast_type = Cast_Type::POINTERS;
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::VOID_TO_FUNCTION_POINTER);
            }
            else if (destination_type->type == Datatype_Type::PRIMITIVE) {
                auto primitive = downcast<Datatype_Primitive>(destination_type);
                if (primitive->primitive_type == Primitive_Type::BOOLEAN) {
            ...
            allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::TO_ANY);
        }
        else if (types_are_equal(source_type, upcast(types.any_type))) {
            cast_type = Cast_Type::FROM_ANY;
            allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::FROM_ANY);
        }
    ...
    // Check if auto-dereference does the trick
    source_type = datatype_get_pointed_to_type(source_type, &result.deref_count);
    if (types_are_equal(destination_type, source_type)) {
        result.cast_type = Cast_Type::NO_CAST;
        return result;
    }
    ...
    // Check for deref to another pointer level, e.g. ***int -> *int
    {
        int destination_type_level;
        auto deref_destination_type = datatype_get_pointed_to_type(destination_type, &destination_type_level);
        if (datatype_is_unknown(deref_destination_type)) {
            result.cast_type = Cast_Type::UNKNOWN;
            return result;
        }

        if (types_are_equal(source_type, deref_destination_type) && auto_dereference)
        {
            if (result.deref_count >= destination_type_level)
            {
                result.cast_type = Cast_Type::NO_CAST;
                result.deref_count = result.deref_count - destination_type_level;
                return result;
            }
        }
    }

    // Check primitive casts
    {
        ...
        case Datatype_Type::ARRAY:
        {
            if (destination_type->type == Datatype_Type::SLICE) {
                ...
                if (types_are_equal(source_array->element_type, dest_slice->element_type)) {
                    ...
                    allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::ARRAY_TO_SLICE);
                }
            }
            break;
        }
        case Datatype_Type::ENUM:
        {
            if (destination_type->type == Datatype_Type::PRIMITIVE)
            {
                ...
                if (primitive->primitive_type == Primitive_Type::INTEGER) {
                    ...
                    allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::ENUM_TO_INT);
                }
            }
            break;
        }
        case Datatype_Type::FUNCTION:
        {
            ...
            {
                cast_type = Cast_Type::POINTERS; // Not sure if we want to have different types for this
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::FUNCTION_POINTER_TO_VOID);
            }
            else if (destination_type->type == Datatype_Type::PRIMITIVE) {
                auto primitive = downcast<Datatype_Primitive>(destination_type);
                if (primitive->primitive_type == Primitive_Type::BOOLEAN) {
        ...
        case Datatype_Type::PRIMITIVE:
        {
            ...
            if (destination_type->type == Datatype_Type::PRIMITIVE)
            {
                ...
                else if (dst_primitive->primitive_type == Primitive_Type::FLOAT && src_primitive->primitive_type == Primitive_Type::FLOAT)
                {
                    ...
                    if (dst_size > src_size) {
                        allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::FLOAT_SIZE_UPCAST);
                    }
                    else {
                        allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::FLOAT_SIZE_DOWNCAST);
                    }
                }
                ...
                destination_type->type == Datatype_Type::FUNCTION)
            {
                ...
                allowed_mode = operator_context_get_cast_mode_option(operator_context, Context_Option::U64_TO_POINTER);
            }
            break;
        }
        default: break;
    ...
    // Check overloads
    {
        ...
        if (overload == 0 && key.left_type->type == Datatype_Type::STRUCT)
        {
            auto struct_type = downcast<Datatype_Struct>(key.left_type);
            if (struct_type->workload != 0 && struct_type->workload->polymorphic_type == Polymorphic_Analysis_Type::POLYMORPHIC_INSTANCE)
            {
                Datatype* base_type = upcast(struct_type->workload->polymorphic.instance.parent->body_workload->struct_type);
                key.left_type = base_type;
                overload = hashtable_find_element(&operator_context->operator_overloads, key);
        ...
        // Test if pointer levels can match
        if (overload != 0)
        {
            if (overload->left_pointer_level == result.deref_count + 1) {
                result.deref_count = 0;
                result.take_address_of = true;
            }
            else if (overload->left_pointer_level > result.deref_count + 1) {
                overload = nullptr;
            }
            else {
                result.deref_count = overload->left_pointer_level;
            }
        }

        // Check if overload has correct cast_type
        if (overload != 0) {
            if ((int)overload->cast_mode > (int)cast_mode) {
                overload = 0;
            }
        }
        ...
        if (overload != 0)
        {
            if (!overload->is_polymorphic) {
                ...
                result.options.custom_cast_function = overload->options.function;
                semantic_analyser_register_function_call(overload->options.function);
                return result;
            }
            else
    ...
    Datatype * initial_type, Expression_Context context, bool log_errors, AST::Expression * expression, Parser::Section error_section)
{
    ...
    case Expression_Context_Type::AUTO_DEREFERENCE: {
        if (!operator_context_get_boolean_option(operator_context, Context_Option::AUTO_DEREFERENCE)) {
            result.cast_type = Cast_Type::NO_CAST;
            return result;
        }
        result.type_afterwards = initial_type;
...
Operator_Context_Value operator_context_value_from_option(Operator_Context * context, Context_Option setting)
{
    Operator_Context_Value result;
    if ((int)setting >= CONTEXT_OPTION_CAST_MODE_START && (int)setting <= CONTEXT_OPTION_CAST_MODE_END) {
        result.is_cast_mode = true;
        result.options.cast_mode = &context->cast_mode_settings[(int)setting - CONTEXT_OPTION_CAST_MODE_START];
    }
    else {
        assert((int)setting >= CONTEXT_OPTION_BOOL_START && (int)setting <= CONTEXT_OPTION_BOOL_END, "");
        result.is_cast_mode = false;
        result.options.bool_value = &context->boolean_settings[(int)setting - CONTEXT_OPTION_BOOL_START];
    }
    return result;
}
    ...
    for (int i = 0; i < context_changes.size; i++)
    {
        ...
        case AST::Context_Change_Type::IMPORT_CONTEXT:
        {
            ...
            for (int j = 0; j < module_analysis->module_node->context_changes.size; j++)
            {
                ...
                assert(other_context->workload != 0, "Should be the case for module contexts!");
                if (other_context->workload == 0) { // To get rid of the warning
                    continue;
                }
                ...
                {
                case AST::Context_Change_Type::ADD_OVERLOAD:
                {
                    auto& overload_info = other_change_info->options.operator_overload;
                    Operator_Overload* overload = hashtable_find_element(&other_context->operator_overloads, overload_info.key);
                    assert(overload != 0, "Should have been set");
                    hashtable_insert_element(&context->operator_overloads, overload_info.key, *overload);
                    if (overload_info.has_commutative_version) {
                        overload = hashtable_find_element(&other_context->operator_overloads, overload_info.commutative_key);
                        assert(overload != 0, "Should have been set");
                        hashtable_insert_element(&context->operator_overloads, overload_info.commutative_key, *overload);
                    }
                    break;
                }
                case AST::Context_Change_Type::SET_OPTION:
                {
                    Operator_Context_Value copy_to_value = operator_context_value_from_option(context, other_change_info->options.option);
                    Operator_Context_Value copy_from_value = operator_context_value_from_option(other_context, other_change_info->options.option);
        ...
        case AST::Context_Change_Type::SET_OPTION:
        {
            auto& arguments = change_node->options.arguments;
            AST::Expression* option_node = nullptr;
            AST::Expression* value_node = nullptr;
            {
                Datatype* parameter_types[2] = { upcast(types.context_option), upcast(types.cast_mode) };
                Array<Datatype*> first_parameter;
                first_parameter.size = 1;
                first_parameter.data = &parameter_types[0];

                int unnamed_argument_count = 0;
                if (arguments_check_for_naming_errors(arguments, &unnamed_argument_count)) {
                    arguments_analyse_in_unknown_context(arguments, first_parameter);
                    break;
                }
                ...
                if (!argument_nodes_match_to_parameters(arguments, callable, unnamed_argument_count, true, upcast(change_node), Parser::Section::ENCLOSURE)) {
                    arguments_analyse_in_unknown_context(arguments, first_parameter);
                    break;
                }
                ...
                ordered_arguments[get_info(arguments[0], false)->parameter_index] = arguments[0]->value;
                ordered_arguments[get_info(arguments[1], false)->parameter_index] = arguments[1]->value;
                option_node = ordered_arguments[0];
                value_node = ordered_arguments[1];
            }

            // Analyse Option
            Context_Option option;
            {
                semantic_analyser_analyse_expression_value(option_node, expression_context_make_specific_type(upcast(types.context_option)));
                auto result = expression_calculate_comptime_value(option_node, "Context Option has to be comptime known");
                if (!result.available) {
                    continue;
                }

                // Check if enum value is valid
                option = upp_constant_to_value<Context_Option>(result.value);
                if ((int)option <= 0 || (int)option >= (int)Context_Option::MAX_ENUM_VALUE) {
                    log_semantic_error("Context option value is invalid", option_node);
                    continue;
                }
            }

            // Analyse setting value
            Operator_Context_Value value_to_change = operator_context_value_from_option(context, option);
            if (value_to_change.is_cast_mode)
            {
                semantic_analyser_analyse_expression_value(value_node, expression_context_make_specific_type(upcast(types.cast_mode)));
                auto result = expression_calculate_comptime_value(value_node, "Cast_Mode has to be comptime known");
                if (!result.available) {
                    continue;
                }
                ...
                if ((int)cast_mode <= 0 || (int)cast_mode > (int)Cast_Mode::NONE) {
                    log_semantic_error("Cast mode value is invalid", value_node);
                    continue;
                }
            ...
            {
                semantic_analyser_analyse_expression_value(value_node, expression_context_make_specific_type(upcast(types.bool_type)));
                auto result = expression_calculate_comptime_value(value_node, "Operator context value has to be comptime known");
                if (!result.available) {
                    continue;
                }
            ...
            info->options.option = option;
            break;
        }
        case AST::Context_Change_Type::ADD_OVERLOAD:
        {
            // Syntax examples: 
            //     context add_overload("+", cast_function = fn, cast_mode = Cast_Mode.IMPLICIT)
            //     context add_overload(.ADDITION, cast_function = fn, cast_mode = Cast_Mode.IMPLICIT)
            ...
            AST::Expression* as_member_access_node = nullptr; // May stay null
            {
                Datatype* context_option_ptr = upcast(types.upp_operator);
                Array<Datatype*> first_parameter;
                first_parameter.size = 1;
                first_parameter.data = &context_option_ptr;
                ...
                Callable callable;
                callable.type = Callable_Type::FUNCTION_POINTER;
                callable.parameter_count = 6;
                callable.only_named_arguments_after_index = callable.parameter_count;
                callable.options.function_pointer_type = types.type_add_overload;
                if (!argument_nodes_match_to_parameters(arguments, callable, unnamed_argument_count, true, upcast(change_node), Parser::Section::ENCLOSURE)) {
                    arguments_analyse_in_unknown_context(arguments, first_parameter);
                    break;
                }
                ...
                for (int j = 0; j < arguments.size; j++) {
                    ...
                    if (arg_info->parameter_index == 0) {
                        operator_argument_node = arguments[j]->value;
                    }
                    else if (arg_info->parameter_index == 1) {
                        function_argument_node = arguments[j]->value;
                    }
                    else if (arg_info->parameter_index == 2) {
                        cast_mode_argument_node = arguments[j]->value;
                    }
                    ...
                    else if (arg_info->parameter_index == 5) {
                        as_member_access_node = arguments[j]->value;
                    }
                    else {
                        panic("");
                    }
            ...
            // Analyse operator 
            bool errors_occured = false;
            Upp_Operator overloaded_operator = Upp_Operator::ADDITION; // Initialize with something
            int expected_argument_count = 2;
            bool commutative_available = true;
            bool polymorphic_valid = false;
            bool cast_mode_available = false;
            bool name_available = false;
            bool member_access_available = false;
            {
                ...
                {
                    semantic_analyser_analyse_expression_value(operator_argument_node, expression_context_make_unknown()); // So that infos are set
                    auto id = operator_argument_node->options.literal_read.options.string;
                    int found_value = -1;
                    for (int i = 0; i < (int)Upp_Operator::MAX_ENUM_VALUE; i++) {
                        if (id == ids.upp_operator_corresponding_tokens[i]) {
                            found_value = i;
                            break;
                        }
                    }
                    ...
                    if (found_value == -1) {
                        log_semantic_error("Operator string is not valid", operator_argument_node);
                        break;
                    }
                    overloaded_operator = (Upp_Operator)found_value;
                }
                else
                {
                ...
                if (!errors_occured)
                {
                    ...
                    {
                    case Upp_Operator::NOT:
                    case Upp_Operator::NEGATE:
                        expected_argument_count = 1;
                        commutative_available = false;
                        break;
                    case Upp_Operator::ARRAY_ACCESS:
                        commutative_available = false;
                        polymorphic_valid = true;
                        break;
                    case Upp_Operator::CAST:
                        expected_argument_count = 1;
                        commutative_available = false;
                        polymorphic_valid = true;
                        cast_mode_available = true;
                        break;
                    case Upp_Operator::DOT_CALL:
                        commutative_available = false;
                        member_access_available = true;
                        polymorphic_valid = true;
                        name_available = true;
                        break;
                    default: break;
            ...
            if (commutative_argument_node != 0)
            {
                ...
                auto value = expression_calculate_comptime_value(commutative_argument_node, "'Commutative' argument must be comptime for overload");
                if (value.available) {
            ...
            bool as_member_access = false;
            if (as_member_access_node != 0) {
                semantic_analyser_analyse_expression_value(as_member_access_node, expression_context_make_specific_type(upcast(types.bool_type)));
                auto value = expression_calculate_comptime_value(as_member_access_node, "'as_member_access' argument must be comptime for dot_calls");
                if (value.available) {
                    as_member_access = upp_constant_to_value<bool>(value.value);
                }
                else {
                    errors_occured = true;
                }

                if (!member_access_available) {
                    log_semantic_error("as_member_access is only available for dot_calls", as_member_access_node);
                    errors_occured = true;
                }
            }
            ...
            // Analyse cast_mode
            Cast_Mode cast_mode = Cast_Mode::EXPLICIT;
            if (cast_mode_argument_node != 0)
            {
                if (!cast_mode_available) {
                    log_semantic_error("Cast mode not available for this Operator", cast_mode_argument_node);
                    errors_occured = true;
                }
                ...
                auto result = expression_calculate_comptime_value(cast_mode_argument_node, "Cast_Mode has to be comptime known");
                if (!result.available) {
                    continue;
                }
            ...
            overload.cast_mode = cast_mode;
            overload.dot_call_as_member_access = as_member_access;
            ...
            if (expr_info->result_type == Expression_Result_Type::FUNCTION)
            {
                ...
                if (overloaded_operator == Upp_Operator::DOT_CALL)
                {
                    ...
                    if (as_member_access && signature->parameters.size > 1) {
                        log_semantic_error("Dot-Call as member access must only have 1 argument", function_argument_node);
                        errors_occured = true;
                    }
            ...
            {
                if (!polymorphic_valid) {
                    log_semantic_error("Polymorphic function not valid for overload of given operator", function_argument_node);
                    errors_occured = true;
                }

                overload.is_polymorphic = true;
                overload.options.polymorphic = expr_info->options.polymorphic_function.base;

                const bool is_dot_call = overloaded_operator == Upp_Operator::DOT_CALL;
                auto poly_base = expr_info->options.polymorphic_function.base->base_info;
                auto function_progress = polymorphic_function_base_to_function_progress(expr_info->options.polymorphic_function.base);
                assert(function_progress->header_workload->base.is_finished, "");
                const int parameter_count = poly_base.parameter_nodes.size;

                // Check that polymorphic base signature is valid for overload
                if (poly_base.return_type_index == -1 && !is_dot_call) {
                    log_semantic_error("Function must have return type for overload", function_argument_node);
                    errors_occured = true;
                }
                else if (overloaded_operator == Upp_Operator::CAST)
                {
                    ...
                    if (return_param.depends_on.size == 0 && !return_param.infos.type->contains_type_template) {
                        key.key_is_type = true;
                        // Note: After custom-cast the destination type must be on the correct pointer level, so we don't do get_pointed_to_type here
                ...
                if (is_dot_call) {
                    if (parameter_count == 0) {
                        log_semantic_error("Dot_calls require at least one argument", function_argument_node);
                        errors_occured = true;
                    }
                    if (as_member_access && parameter_count > 1) {
                        log_semantic_error("Dot_calls with as_member_access must have exactly one parameter", function_argument_node);
                        errors_occured = true;
                    }
                }
                else
                ...
                if (!errors_occured)
                {
                    ...
                    {
                        key.left_type = datatype_get_pointed_to_type(param.infos.type, &overload.left_pointer_level);
                        if (key.left_type->type != Datatype_Type::STRUCT_INSTANCE_TEMPLATE) {
                            log_semantic_error("For polymorphic overloads the first parameter must be a struct instance-template", function_argument_node);
                            errors_occured = true;
                        }
                        else {
                            key.left_type = upcast(downcast<Datatype_Struct_Instance_Template>(key.left_type)->struct_base->body_workload->struct_type);
                        }
                    }
                ...
                log_semantic_error("Overload function argument must be function or polymorphic-function", function_argument_node);
                errors_occured = true;
            }
            ...
            // Try to insert into operator context
            {
                ...
                if (worked)
                {
                    info->is_valid_for_import = true; // Change that at some point
                    info->options.operator_overload.key = key;
                    info->options.operator_overload.has_commutative_version = false;
                }
                else {
            ...
            if (is_commutative && commutative_available && key.key_is_type && !types_are_equal(key.left_type, key.options.right_type))
            {
                ...
                other_overload.left_pointer_level = overload.right_pointer_level;
                other_overload.right_pointer_level = overload.left_pointer_level;
                other_overload.switch_left_and_right = true;
                bool worked = hashtable_insert_element(&context->operator_overloads, other_key, other_overload);
                if (worked)
                {
                    info->options.operator_overload.has_commutative_version = true;
                    info->options.operator_overload.commutative_key = other_key;
                }
                else {
...
#define EXIT(flow_result) { info->flow = flow_result; return flow_result; };
    ...
    case AST::Statement_Type::BINOP_ASSIGNMENT:
    {
        ...
        if (!types_are_valid)
        {
            ...
            if (overload != nullptr)
            {
                assert(!overload->is_polymorphic, "Binop overloads cannot be polymorphic currently");
                assert(overload->options.function->signature->return_type.available, "");
                if (!types_are_equal(left_type, overload->options.function->signature->return_type.value)) {
                    log_semantic_error(
                ...
                info->specifics.overload.function = overload->options.function;
                info->specifics.overload.switch_arguments = overload->switch_left_and_right;
                semantic_analyser_register_function_call(overload->options.function);
                types_are_valid = true;
            }
        }
