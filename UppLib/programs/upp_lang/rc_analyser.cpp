...
Symbol* symbol_table_define_symbol(Symbol_Table* symbol_table, RC_Analyser* analyser, String* id, Symbol_Type type, AST_Node* definition_node)
{
    ...
    if (found_symbol != 0) {
        rc_analyser_log_error(analyser, Symbol_Error_Type::SYMBOL_ALREADY_EXISTS, found_symbol, definition_node);
...
RC_Expression* rc_expression_create_empty(RC_Analyser* analyser, RC_Expression_Type type, AST_Node* origin_node)
{
    ...
    hashtable_insert_element(&analyser->mapping_expressions_to_ast, expression, origin_node);
    return expression;
}
...
RC_Statement* rc_statement_create_empty(RC_Analyser* analyser, RC_Statement_Type type, AST_Node* origin_node)
{
    ...
    statement->type = type;
    hashtable_insert_element(&analyser->mapping_statements_to_ast, statement, origin_node);
    return statement;
}
...
void rc_expression_destroy(RC_Expression* expression)
{
    ...
    case RC_Expression_Type::SYMBOL_READ:
        delete expression->options.symbol_read;
        break;
...
ANALYSER FUNCTIONS
*/
RC_Analysis_Item* rc_analysis_item_create_empty(RC_Analysis_Item_Type type, RC_Analysis_Item* parent_item)
{
    RC_Analysis_Item* item = new RC_Analysis_Item;
    item->dependencies_items = dynamic_array_create_empty<RC_Analysis_Item*>(1);
    item->dependencies_symbols = dynamic_array_create_empty<RC_Symbol_Read*>(1);
    item->type = type;
    if (parent_item != 0) {
        dynamic_array_push_back(&parent_item->dependencies_items, item);
    }
    return item;
}
...
void rc_analysis_item_destroy(RC_Analysis_Item* item)
{
    for (int i = 0; i < item->dependencies_items.size; i++) {
        rc_analysis_item_destroy(item->dependencies_items[i]);
    }
    dynamic_array_destroy(&item->dependencies_items);
    for (int i = 0; i < item->dependencies_symbols.size; i++) {
        delete (item->dependencies_symbols[i]);
    }
    dynamic_array_destroy(&item->dependencies_symbols);
...
void rc_analyser_reset(RC_Analyser* analyser, Compiler* compiler)
{
    ...
    if (analyser->root_item != 0) {
        rc_analysis_item_destroy(analyser->root_item);
        analyser->root_item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::ROOT, 0);
    }
...
RC_Expression* rc_analyser_analyse_expression(RC_Analyser* analyser, AST_Node* expression_node)
{
    ...
    case AST_Node_Type::MODULE:
    {
        ...
        rc_analyser_analyse_definitions(analyser, expression_node);
    ...
    {
        RC_Analysis_Item* function_item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::FUNCTION, analyser->analysis_item);
        ...
        // Analyse body
        RC_Analysis_Item* body_item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::FUNCTION_BODY, 0);
    ...
    case AST_Node_Type::FUNCTION_SIGNATURE:
    {
        ...
        {
            assert(parameter_node->type == AST_Node_Type::PARAMETER, "");
            RC_Parameter param;
    ...
    {
        RC_Analysis_Item* struct_item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::STRUCTURE, analyser->analysis_item);
    ...
    case AST_Node_Type::EXPRESSION_IDENTIFIER:
    {
        ...
        read->symbol = 0;
        read->type = Symbol_Dependency_Type::NORMAL;
    ...
    case AST_Node_Type::EXPRESSION_FUNCTION_CALL: {
        ...
        while (argument_node != 0) {
            dynamic_array_push_back(&result_expr->options.function_call.arguments, rc_analyser_analyse_expression(analyser, argument_node));
    ...
    case AST_Node_Type::EXPRESSION_AUTO_STRUCT_INITIALIZER:
    {
        ...
        while (init_node != 0) {
            ...
            member_init.member_id = init_node->id;
    ...
    case AST_Node_Type::EXPRESSION_BAKE: {
        RC_Analysis_Item* bake_item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::BAKE, analyser->analysis_item);
...
void rc_analyser_analyse_symbol_definition_node(RC_Analyser* analyser, AST_Node* definition_node)
{
    ...
    case AST_Node_Type::VARIABLE_DEFINITION:
    {
        RC_Analysis_Item* item = rc_analysis_item_create_empty(RC_Analysis_Item_Type::DEFINITION, analyser->analysis_item);
