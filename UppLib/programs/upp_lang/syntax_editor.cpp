...
String syntax_token_as_string(Syntax_Token token, Syntax_Editor* editor)
{
	switch (token.type)
	{
...
void syntax_lexer_parse_line(Syntax_Line* line, Syntax_Editor* editor)
{
	...
	*/
	...
	// Determine if cursor is after token/inside token
	int cursor_offset = 0;
	int after_token_spaces = 0; // Either 0, 1 or 2
	{
		auto& info = tokens[cursor_token].info;
		cursor_offset = cursor - info.char_start;

		// Check if cursor is after token, and if it is, if we have a 2 space situation
		// '+   | what'
		if (cursor_offset > info.char_length) {
			after_token_spaces = 1;
			if (cursor + 1 <= text.size && text[cursor + 1] == ' ') {
				after_token_spaces = 2;
			}
			if (!is_space_critical(tokens[cursor_token].type)) {
				// No spaces allowed after non space critical token
				after_token_spaces = 0; 
				cursor_offset = info.char_start + info.char_length;
			}
		}

		// Inside/at end of token, count spaces between cursor and diff that for the cursor offset
		// "*  = |="  -> "*=|="
		{
			String between = string_create_substring_static(&text, info.char_start, info.char_start + info.char_length);
			int empty_between = 0;
			String should_be = syntax_token_as_string(tokens[cursor_token], editor);
			int should_index = 0;
			int text_index = info.char_start;
			while (should_index < should_be.size) {
				if (text[text_index] == should_be[should_index]) {
					text_index++;
					should_index++;
				}
				else {
					if (cursor < text_index) {
						cursor_offset--;
					}
					text_index++;
				}
	...
	for (int i = 0; i < tokens.size; i++)
	{
		...
		Syntax_Token_Type next_type = Syntax_Token_Type::UNEXPECTED_CHAR;
		if (i > 0) {
			previous_type = tokens[i - 1].type;
		}
		if (i + 1 < tokens.size) {
		...
		case Syntax_Token_Type::KEYWORD:
		case Syntax_Token_Type::LITERAL:
			if (is_space_critical(next_type)) {
				info.critical_space_count = 1;
				if (i == cursor_token && after_token_spaces != 0) {
					info.critical_space_count = after_token_spaces;
				}
			}
			else if (i == cursor_token && after_token_spaces != 0) {
				info.critical_space_count = 1;
			}
			break;
		case Syntax_Token_Type::OPERATOR:
	...
	for (int i = 0; i < tokens.size; i++)
	{
		...
		String str = syntax_token_as_string(token, editor);
		token.info.char_length = str.size;

		string_append_string(&new_text, &str);
		for (int j = 0; j < info.critical_space_count; j++) {
...
void line_print_tokens(Syntax_Line* line, Syntax_Editor* editor)
{
	...
	for (int i = 0; i < line->tokens.size; i++)
	{
		...
		String substr = syntax_token_as_string(token, editor);
		string_append_formated(&output, " ");
...
void syntax_editor_render_line(Syntax_Editor* editor, int line_index)
{
	...
	*/
	...
	for (int i = 0; i < line.tokens.size; i++)
	{
		...
		if (line_index == editor->line_index && cursor >= info.char_start && 
			((i + 1 < line.tokens.size && cursor < line.tokens[i+1].info.char_start) || i + 1 >= line.tokens.size)) {
			cursor_pos = pos + (cursor - info.char_start);
		}

		if (info.format_space_before) pos += 1;
		String str = syntax_token_as_string(token, editor);
		syntax_editor_draw_string(editor, str, token.type == Syntax_Token_Type::KEYWORD ? Syntax_Color::KEYWORD : Syntax_Color::TEXT, line_index, pos);
		pos += str.size + info.critical_space_count;
		if (info.format_space_after) pos += 1;
	}
	if (line_index == editor->line_index) {
		if (editor->mode == Editor_Mode::NORMAL) {
			syntax_editor_draw_character_box(editor, Syntax_Color::COMMENT, line_index, cursor_pos);
