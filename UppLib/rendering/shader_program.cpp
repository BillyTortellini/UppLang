...
Shader_Program* shader_program_create(std::initializer_list<const char*> shader_filepaths)
{
    ...
    shader_program->file_listener = rendering_core.file_listener;
    ...
    // Setup file watchers
    {
        ...
        {
            shader_program->watched_files.data[i] = file_listener_add_file(rendering_core.file_listener,
        ...
        if (watched_file_count != shader_program->shader_filepaths.size) {
            for (int i = 0; i < watched_file_count; i++) {
                file_listener_remove_file(rendering_core.file_listener, shader_program->watched_files.data[i]);
            }
...
void shader_program_draw_mesh(Shader_Program* program, Mesh_GPU_Buffer* mesh, std::initializer_list<Uniform_Value> uniforms)
{
    ...
    for (auto& uniform_value : uniforms) {
        shader_program_set_uniform_value(program, uniform_value);
    }
...
void shader_program_bind(Shader_Program * program) {
    opengl_state_bind_program(program->program_id);
}
...
bool shader_program_set_uniform_value(Shader_Program* program, Uniform_Value value) { return true; }
Shader_Variable_Information* shader_program_find_shader_variable_information_by_name(Shader_Program* program, const char* name_handle) { return 0; }
bool shader_program_check_compatability_with_mesh(Shader_Program* shader_program, Mesh_GPU_Buffer* mesh) { return true; }
